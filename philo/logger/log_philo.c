 ...
 */
static int	_free_offset_id(char *offset, char *id, int exit_status)
{
	if (offset != NULL)
		safe_free(offset);
	if (id != NULL)
 ...
 */
static int	_start_log_entry(t_philo *philo, char *offset, char *id,
	enum e_ansi log_lvl)
{
	if (id == NULL)
		return (EXIT_FAILURE);
	...
	if (sim_has_ended(philo->params) != 0)
		return (pthread_mutex_unlock(&philo->params->log_mtx), EXIT_FAILURE);
...
int	log_philo(enum e_philo_log log, t_philo *philo)
{
	...
	if (logger(_get_philo_log_msg(log), log_lvl, INFO) < 0)
		return (pthread_mutex_unlock(&philo->params->log_mtx),
			_free_offset_id(offset, id, EXIT_FAILURE));
